...
// The build/test commands a plugin declares to the canonical Signal K
// plugin-ci reusable workflow, as read from its own caller workflow file.
export interface DeclaredCiCommands {
  build?: string;
  test?: string;
}
...
const REUSABLE_PLUGIN_CI =
  "signalk/signalk-server/.github/workflows/plugin-ci.yml";
...
function isPlausibleCommand(value: string): boolean {
...
export function parseDeclaredCiCommands(sourceDir: string): DeclaredCiCommands {
  ...
  for (const file of files) {
    ...
    if (declared.build || declared.test) return declared;
  }
  return {};
}
...
function commandsFromWorkflow(content: string): DeclaredCiCommands {
  ...
  const result: DeclaredCiCommands = {};
  let inWith = false;
  let withIndent = -1;
  for (let i = blockStart; i < blockEnd; i++) {
    ...
    if (indent <= usesIndent) break; // `with:` block ended
    if (withIndent === -1) withIndent = indent;
    if (indent !== withIndent) continue; // skip nested/continuation lines

    const cm = /^\s*(build|test)-command:\s*(.+?)\s*$/.exec(line);
    if (cm) {
      const value = parseScalar(cm[2]);
      if (isPlausibleCommand(value)) {
        if (cm[1] === "build") result.build = value;
        else result.test = value;
      }
    }
  }
